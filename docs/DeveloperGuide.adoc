...
** Cons: Any page that is loaded on `_BrowserPanel_` will be lost during the operation.
...
** Cons: We are not entirely sure if this implementation is possible.
* **Alternative 3:** Reload the default page *and* reload any page currently loaded on `_BrowserPanel_`.
** Pros: It is possible to keep the status of the `_BrowserPanel_`, in other words, any page loaded may still retain.
** Cons: This approach is obviously harder to implement. It will also send an additional request to the server of the website.
* **Alternative 4:** Save any page currently loaded on `_BrowserPanel_`, reload the default page then load the saved page.
** Pros: This is the ideal method.
** Cons: Because this is the ideal output, it is very hard to implement.
